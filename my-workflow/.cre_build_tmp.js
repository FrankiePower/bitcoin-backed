...
var configSchema = exports_external.object({
  ...
  minBPSDeltaForRebalance: exports_external.number().min(0),
  evms: exports_external.array(exports_external.object({
    ...
    protocolSmartWalletAddress: exports_external.string(),
    chainName: exports_external.string(),
    gasLimit: exports_external.string()
  }))
});
var aprInRAYToAPR = (apr) => parseFloat(formatUnits(apr, 27));
var aprInRAYToAPY = (apr) => Math.exp(parseFloat(formatUnits(apr, 27))) - 1;
var RAY_TO_BPS_DIVISOR = 100000000000000000000000n;
var aprDiffToBps = (diffRay) => diffRay / RAY_TO_BPS_DIVISOR;
var getEvmClientForChain = (evmCfg) => {
...
var readCurrentLiquidityRate = (runtime2, evmCfg, evmClient) => {
  const callData = encodeFunctionData({
    abi: MockPool,
    functionName: "getReserveData",
    args: [evmCfg.assetAddress]
  });
  const callResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmCfg.poolAddress,
      data: callData
    }),
...
var readBalanceInPool = (runtime2, evmCfg, evmClient) => {
  const callData = encodeFunctionData({
    abi: MockPool,
    functionName: "balanceOf",
    args: [evmCfg.protocolSmartWalletAddress, evmCfg.assetAddress]
  });
  const callResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
...
var buildPoolForChain = (runtime2, evmCfg) => {
  runtime2.log(`Reading APY for chain ${evmCfg.chainName} | pool ${evmCfg.poolAddress} | asset ${evmCfg.assetAddress}`);
  const evmClient = getEvmClientForChain(evmCfg);
  const currentLiquidityRate = readCurrentLiquidityRate(runtime2, evmCfg, evmClient);
  runtime2.log(`Reserve data [${evmCfg.chainName}] currentLiquidityRate: ${currentLiquidityRate}`);
  const balanceInPool = readBalanceInPool(runtime2, evmCfg, evmClient);
  runtime2.log(`Balance in pool [${evmCfg.chainName}]: ${balanceInPool}`);
  const apy = aprInRAYToAPY(currentLiquidityRate);
  const apr = aprInRAYToAPR(currentLiquidityRate);
  runtime2.log(`Supply yield [${evmCfg.chainName}] APY%: ${(apy * 100).toFixed(6)}, APR%: ${(apr * 100).toFixed(6)}`);
  return {
...
var findBestPool = (pools, log) => {
  ...
  if (!best || best.APR <= 0n)
    throw new Error("Best APY unset or <= 0");
  return best;
};
var getChainSelectorFor = (chainName) => {
  const network248 = getNetwork({
    chainFamily: "evm",
  ...
  return network248.chainSelector.selector;
};
var shouldRebalance = (maxAPR, curAPR, minBpsDelta) => {
  const diff = maxAPR - curAPR;
  const diffBps = aprDiffToBps(diff);
  return { ok: diffBps >= BigInt(minBpsDelta), diffBps };
};
var performRebalance = (runtime2, evmCfg, amount, bestChainSelector, bestProtocolSmartWallet) => {
  const evmClient = getEvmClientForChain(evmCfg);
  const reportData = encodeAbiParameters(parseAbiParameters("address asset, uint256 amount, uint64 destinationChainSelector, address destinationProtocolSmartWallet"), [evmCfg.assetAddress, amount, bestChainSelector, bestProtocolSmartWallet]);
  ...
  runtime2.log(`Write report transaction succeeded on ${evmCfg.chainName} txHash: ${bytesToHex(txHash)}`);
};
var doHighestSupplyAPY = (runtime2) => {
  ...
  runtime2.log("Reading supply APYs...");
  const pools = config.evms.map((e) => buildPoolForChain(runtime2, e));
  const bestPool = findBestPool(pools, runtime2.log);
  runtime2.log(`Found best APY: ${bestPool.APY} on chain ${bestPool.chainName}`);
  ...
  for (const evmCfg of config.evms) {
    if (evmCfg.chainName === bestPool.chainName)
      continue;
    runtime2.log(`Rebalancing from ${evmCfg.chainName} -> ${bestPool.chainName} (selector ${bestChainSelector})`);
    const evmClient = getEvmClientForChain(evmCfg);
    const balance = readBalanceInPool(runtime2, evmCfg, evmClient);
    if (balance === 0n) {
      runtime2.log(`No balance to rebalance on chain ${evmCfg.chainName}, skipping.`);
      continue;
    }
    const curPool = pools.find((p) => p.chainName === evmCfg.chainName);
    if (!curPool)
      throw new Error(`Pool info not found for chain ${evmCfg.chainName}`);
    const { ok, diffBps } = shouldRebalance(bestPool.APR, curPool.APR, config.minBPSDeltaForRebalance);
    if (!ok) {
      runtime2.log(`APY diff below threshold: diff=${diffBps}, min=${config.minBPSDeltaForRebalance} → skipping.`);
      continue;
    }
    runtime2.log(`Rebalancing supply from ${evmCfg.chainName} to ${bestPool.chainName} | balance=${balance}`);
    performRebalance(runtime2, evmCfg, balance, bestChainSelector, bestPool.protocolSmartWalletAddress);
    newBestBalance += balance;
    totalRebalancedAmount += balance;
  }
  runtime2.log(`Rebalancing complete | Old balance: ${bestPool.balance} | New balance: ${newBestBalance} | Amount rebalanced: ${totalRebalancedAmount} | Chain: ${bestPool.chainName}`);
  return "";
};
var onCronTrigger = (runtime2, payload) => {
  ...
  runtime2.log("Running CronTrigger for supply APY rebalance");
  return doHighestSupplyAPY(runtime2);
};
